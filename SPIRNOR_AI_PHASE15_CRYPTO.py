#!/usr/bin/env python3
"""
SPIRNOR AI Phase 15: Cryptographic Modular Arithmetic

Building on Phase 8's CRT theory (C=2pi/p encodes exact mod-p residues)
and Phase 11's compositional reasoning, this phase tests SPIRNOR on
fundamental cryptographic operations:
  - Modular exponentiation (the core crypto primitive)
  - RSA encryption (CRT over two primes)
  - ElGamal encryption (multiplication + exponentiation)
  - Diffie-Hellman key exchange (multi-step modular reasoning)
  - Discrete logarithm (the inverse "hard problem")
  - Power tower / iterated exponentiation (nested composition)

8 Tasks across 6 Crypto Families:
  1. ModPow7:         (a,b) -> a^b mod 7          [7 classes]
  2. ModPow11:        (a,b) -> a^b mod 11         [11 classes]
  3. ModPow30:        (a,b) -> a^b mod 30         [30 classes]
  4. RSAEncrypt77:    (m)   -> m^7 mod 77         [77 classes]
  5. ElGamalEncrypt7: (m,k) -> m * 3^k mod 7      [7 classes]
  6. DiffieHellman7:  (a,b) -> 3^(a*b) mod 7      [7 classes]
  7. DiscreteLog11:   (y)   -> x: 2^x = y mod 11  [10 classes]
  8. PowerTower7:     (a,b) -> a^(a^b) mod 7      [7 classes]

Theory:
  - Fermat's Little Theorem: a^b mod p = a^(b mod (p-1)) mod p
  - SPIRNOR encodes a mod p AND b mod (p-1) directly in features
  - CRT: a^b mod 30 = CRT(a^b mod 2, a^b mod 3, a^b mod 5)
  - RSA: m^7 mod 77 = CRT(m^7 mod 7, m^7 mod 11)
...
MAX_SEQ = 2  # Maximum number inputs per example (all tasks use 1 or 2)

# Discrete log lookup table for g=2, p=11
# 2 is a primitive root mod 11, order = 10
# 2^x mod 11: {0:1, 1:2, 2:4, 3:8, 4:5, 5:10, 6:9, 7:7, 8:3, 9:6}
DLOG_TABLE_11 = {}
for _x in range(10):
    _y = pow(2, _x, 11)
    DLOG_TABLE_11[_y] = _x
...
# ============================================================
# TASK DEFINITIONS (8 tasks across 6 crypto families)
# ============================================================

TASKS = OrderedDict([
    # Family 1: Modular Exponentiation
    ('modpow7',  {'n_inputs': 2, 'n_classes': 7,  'name': 'ModPow7',  'family': '1_ModExp'}),
    ('modpow11', {'n_inputs': 2, 'n_classes': 11, 'name': 'ModPow11', 'family': '1_ModExp'}),
    ('modpow30', {'n_inputs': 2, 'n_classes': 30, 'name': 'ModPow30', 'family': '1_ModExp'}),
    # Family 2: RSA Encryption
    ('rsa77',    {'n_inputs': 1, 'n_classes': 77, 'name': 'RSA77',    'family': '2_RSA'}),
    # Family 3: ElGamal Encryption
    ('elgamal7', {'n_inputs': 2, 'n_classes': 7,  'name': 'ElGamal7', 'family': '3_ElGamal'}),
    # Family 4: Diffie-Hellman Key Exchange
    ('dh7',      {'n_inputs': 2, 'n_classes': 7,  'name': 'DH7',      'family': '4_DH'}),
    # Family 5: Discrete Logarithm
    ('dlog11',   {'n_inputs': 1, 'n_classes': 10, 'name': 'DLog11',   'family': '5_DLog'}),
    # Family 6: Iterated/Nested Operations
    ('tower7',   {'n_inputs': 2, 'n_classes': 7,  'name': 'Tower7',   'family': '6_Tower'}),
])
...
def generate_dataset(n_per_task, num_range, rng):
    """Generate training/eval data for all 8 Phase 15 crypto tasks."""
    lo, hi = num_range
    all_task_ids = []
    all_numbers = []
    all_labels = []
    ...
    for task_name in TASK_LIST:
        ...
        elif task_name == 'modpow11':
            # a^b mod 11 (11 classes)
            for _ in range(n_per_task):
                a = int(rng.randint(lo, hi + 1))
                b = int(rng.randint(lo, hi + 1))
                label = pow(a, b, 11)
        ...
        elif task_name == 'rsa77':
            # m^7 mod 77 (77 classes), N=77=7*11, e=7
            for _ in range(n_per_task):
                m = int(rng.randint(lo, hi + 1))
                label = pow(m, 7, 77)
                all_task_ids.append(task_id)
                all_numbers.append([m, 0])  # single input, pad second
                all_labels.append(label)

        elif task_name == 'elgamal7':
            # m * 3^k mod 7 (7 classes), g=3, p=7
            for _ in range(n_per_task):
                m = int(rng.randint(lo, hi + 1))
                ...
                label = (m * shared) % 7
                all_task_ids.append(task_id)
                all_numbers.append([m, k])
                all_labels.append(label)

        elif task_name == 'dh7':
            # 3^(a*b) mod 7 (7 classes), g=3, p=7
            for _ in range(n_per_task):
                a = int(rng.randint(lo, hi + 1))
                b = int(rng.randint(lo, hi + 1))
                label = pow(3, a * b, 7)
        ...
        elif task_name == 'dlog11':
            # Discrete log: find x where 2^x = y mod 11 (10 classes)
            # Skip y where y % 11 == 0 (no solution exists)
            count = 0
            attempts = 0
            while count < n_per_task and attempts < n_per_task * 5:
                attempts += 1
                y = int(rng.randint(lo, hi + 1))
                y_mod = y % 11
                if y_mod == 0:
                    continue
                label = DLOG_TABLE_11[y_mod]
                all_task_ids.append(task_id)
                all_numbers.append([y, 0])  # single input, pad second
                all_labels.append(label)
                count += 1

        elif task_name == 'tower7':
            # a^(a^b) mod 7 (7 classes)
            # By Fermat: a^(a^b) mod 7 = a^((a^b) mod 6) mod 7
            for _ in range(n_per_task):
                a = int(rng.randint(lo, hi + 1))
                b = int(rng.randint(lo, hi + 1))
                if a % 7 == 0:
                    label = 0  # 7|a => a^k = 0 mod 7 for k >= 1
                else:
                    inner = pow(a, b, 6)  # a^b mod (7-1) by Fermat
                    label = pow(a, inner, 7)
                all_task_ids.append(task_id)
                all_numbers.append([a, b])
                all_labels.append(label)
...
def run_part_a():
    print("=" * 70)
    print("PHASE 15 PART A: CRYPTOGRAPHIC MODULAR ARITHMETIC (d_model=128)")
    print("2 embedding configs x 8 crypto tasks")
    print("=" * 70)
    ...
    for task_name in TASK_LIST:
        ...
        print(f"    {task_name:12s} labels: {unique.min().item()}-{unique.max().item()}, "
...
def run_part_b():
    print("\n" + "=" * 70)
    print("PHASE 15 PART B: SCALING TEST (d_model=256)")
...
def print_summary(a_results, b_results):
    ...
    families = OrderedDict([
        ('1_ModExp',  {'name': 'Modular Exponentiation', 'tasks': ['modpow7', 'modpow11', 'modpow30']}),
        ('2_RSA',     {'name': 'RSA Encryption',         'tasks': ['rsa77']}),
        ('3_ElGamal', {'name': 'ElGamal Encryption',     'tasks': ['elgamal7']}),
        ('4_DH',      {'name': 'Diffie-Hellman',         'tasks': ['dh7']}),
        ('5_DLog',    {'name': 'Discrete Logarithm',     'tasks': ['dlog11']}),
        ('6_Tower',   {'name': 'Power Tower',            'tasks': ['tower7']}),
    ])
    ...
    if 'spirnor_rational5' in a_results:
        ...
        for task_name in TASK_LIST:
            ...
            print(f"  {TASKS[task_name]['name']:12s}: [{val_str}] drop={drop:+.3f}")
    ...
    if 'spirnor_rational5' in a_results:
        print(f"\n  CRYPTO TASK VALIDATION (SPIRNOR @ 100K-500K):")
        for task_name in TASK_LIST:
            ...
            bar = '#' * int(v * 30)
            print(f"    {TASKS[task_name]['name']:12s}: {v:.4f} [{status}] {bar}")
...
def save_results(a_results, b_results):
    ...
    save_data = {
        'phase': 'Phase 15: Cryptographic Modular Arithmetic',
        'config': {
            'part_a': {
        ...
        'task_families': {
            '1_ModExp':  ['modpow7', 'modpow11', 'modpow30'],
            '2_RSA':     ['rsa77'],
            '3_ElGamal': ['elgamal7'],
            '4_DH':      ['dh7'],
            '5_DLog':    ['dlog11'],
    ...
    save_path = 'SPIRNOR_AI_PHASE15_RESULTS.json'
