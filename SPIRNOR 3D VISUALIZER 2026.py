...
class SPIRNORVisualizer:
    ...
    def _build_ui(self):
        ...
        self.ax_cslider = self.fig.add_axes([panel_l, 0.885, panel_w, 0.025])
        self.ax_cslider.set_facecolor(BG_WIDGET)
        self.c_slider = Slider(self.ax_cslider, "", 0.1, 10.0,
        ...
        self.ax_rslider = self.fig.add_axes([panel_l, 0.84, panel_w, 0.025])
        self.ax_rslider.set_facecolor(BG_WIDGET)
        self.range_slider = Slider(self.ax_rslider, "", 500, LIMIT,
        ...
        self.ax_chk = self.fig.add_axes([panel_l, 0.555, 0.18, 0.14])
        self.ax_chk.set_facecolor(BG_PANEL)
        ...
        self.ax_cmap = self.fig.add_axes([panel_l + 0.20, 0.585, 0.15, 0.11])
        self.ax_cmap.set_facecolor(BG_PANEL)
        self.cmap_radio = RadioButtons(
            self.ax_cmap, ["plasma", "inferno", "turbo", "cool"],
            active=0,
        )
        ...
        self.action_btns = []          # ← prevent garbage collection
        self.action_btn_axes = []      # ← keep axes alive too
        for i, (label, cb) in enumerate(btn_defs):
            ...
            self.action_btn_axes.append(bx)
            if "Animate" in label:
                self.anim_btn = btn
    ...
    def _fft_analysis(self, event):
        try:
            ...
            fft_fig = plt.figure(figsize=(13, 10), facecolor=BG_DARK)
            fft_fig.suptitle(f"SPIRNOR FFT  ·  {tag}  ·  C = {self.current_C:.4f}",
                             fontsize=13, fontweight="bold", color=ACCENT)

            # ---- Panel 1: 3D peaks ----
            ax0 = fft_fig.add_subplot(2, 2, 1, projection="3d")
            ...
            if len(idx):
                ...
                fft_fig.colorbar(sc, ax=ax0, shrink=0.65, label="log|FFT|")
            ax0.set_title("3D Peaks", color=TEXT, fontsize=10)
            ax0.set_xlabel("fX", fontsize=8)
            ax0.set_ylabel("fY", fontsize=8)
            ax0.set_zlabel("fZ", fontsize=8)

            # ---- Panels 2-4: 2D slices ----
            slices = [
            ...
            for i, (data, title) in enumerate(slices):
                ax = fft_fig.add_subplot(2, 2, i + 2)
                ax.set_facecolor(BG_DARK)
                im = ax.imshow(data, cmap="inferno", origin="lower")
                ax.set_title(title, color=TEXT, fontsize=10)
                fft_fig.colorbar(im, ax=ax, shrink=0.8)
                ax.tick_params(colors=TEXT_DIM, labelsize=7)

            fft_fig.tight_layout(rect=[0, 0, 1, 0.95])
            fft_fig.show()
            print(f"FFT complete – {len(idx)} peaks above 99.5th percentile")
