...
- Extensive empirical evidence across 13 phases, 29 tasks, 6 domain types (including temporal), and 6 model architectures showing consistent OOD advantage where periodic/modular structure is relevant.
...
We have presented SPIRNOR, a deterministic numeric embedding based on logarithmic spiral mappings parameterized by mathematical constants. Through thirteen experimental phases spanning 29 classification tasks plus autoregressive arithmetic, 6 domain types (including temporal pattern recognition), 6 architectures, scales up to 400,000 training examples, and rigorous multi-seed statistical validation, we demonstrate that SPIRNOR provides:
...
These results establish that mathematical structure — specifically, periodic patterns encoded through angular projections — provides a powerful inductive bias for numeric and temporal reasoning. The evolution from the original SPIRNOR equation (Phase 1) through ablation (Phases 7-7B) to the rational fraction framework (Phase 8) to domain expansion (Phase 9), compositional reasoning (Phase 11), and temporal application (Phase 12) represents a progression from empirical observation to theoretical understanding to practical utility: the embedding works because it computes exact periodic information, and the optimal constants are those matched to the periodicities of the target domain. Phase 9's ModMul7 result — 100% OOD accuracy for a task directly testing mod-7 computation — provided the first direct validation of this theory. Phase 11's CRT validation (Mod30 = 100% at all ranges) provided the definitive confirmation. Phase 12's temporal results generalize the principle: replacing prime constants with temporal-period constants yields 7/8 tasks at 100% OOD up to 50-year extrapolation, demonstrating that the framework is domain-agnostic. The insight connects neural numeric representation to classical number theory (the Chinese Remainder Theorem), Fourier analysis (exact frequency matching), and a general principle: the best inductive biases are not approximate statistical regularities but exact mathematical invariants of the target domain.
