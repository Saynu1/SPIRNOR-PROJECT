...
We conduct a systematic thirteen-phase experimental program demonstrating SPIRNOR's effectiveness:
...
- Extensive empirical evidence across 14 phases, 29 tasks, 6 domain types (including temporal), 6 model architectures, and 6 encoding methods showing consistent OOD advantage where periodic/modular structure is relevant.
...
1. **Task scope and domain knowledge requirement**: SPIRNOR dominates on classification tasks with periodic/modular structure, spanning both number theory (GCD, SPF, Coprime, ModMul7, IsPrime: 1.4-6.5x; compositional: Mod30, Diophantine, SetAllDivisible: 1.3-142.9x) and temporal pattern recognition (HourOfDay, DayOfWeek, IsBusinessHour: 1.5-167x). It yields to learned embeddings on pure magnitude tasks (Compare), and provides no OOD benefit for autoregressive digit-level generation (Phase 10: 0% exact match for all configs). Phase 12 demonstrates that SPIRNOR generalizes beyond number theory to any domain with known periodicities, but **the advantage requires domain knowledge to select appropriate constants**. Phase 13 proves this is not merely a convenience — it is fundamental: Time2Vec and Learned Fourier methods, which attempt to *learn* frequencies from data, achieve only 33-36% OOD (worse than learned embeddings at 40.1%). The frequencies cannot be discovered through gradient descent in reasonable training time. SPIRNOR is a domain-appropriate inductive bias that requires upfront period knowledge, not a universal embedding improvement.
...
4. **Scale**: Phase 9 extends to d_model=256 (3.7M params, +34.5 pp advantage), Phase 11 confirms at d_model=256 (5.3M params, +57.4 pp advantage), and Phase 12 maintains near-perfect performance at d_model=256 (5.3M params, 99.6% avg OOD). Whether this trend continues to modern LLM scale (billions of parameters) is unknown, though the consistently positive scaling trajectory is encouraging.
...
We have presented SPIRNOR, a deterministic numeric embedding based on logarithmic spiral mappings parameterized by mathematical constants. Through fourteen experimental phases spanning 29 classification tasks plus autoregressive arithmetic, 6 domain types (including temporal pattern recognition), 6 architectures, 6 encoding methods (Phase 13), scales up to 400,000 training examples, and rigorous multi-seed statistical validation, we demonstrate that SPIRNOR provides:
...
These results establish that mathematical structure — specifically, periodic patterns encoded through angular projections — provides a powerful inductive bias for numeric and temporal reasoning. The evolution from the original SPIRNOR equation (Phase 1) through ablation (Phases 7-7B) to the rational fraction framework (Phase 8) to domain expansion (Phase 9), compositional reasoning (Phase 11), temporal application (Phase 12), and baseline comparison (Phase 13) represents a progression from empirical observation to theoretical understanding to practical validation: the embedding works because it computes exact periodic information, and the optimal constants are those matched to the periodicities of the target domain. Phase 9's ModMul7 result — 100% OOD accuracy for a task directly testing mod-7 computation — provided the first direct validation of this theory. Phase 11's CRT validation (Mod30 = 100% at all ranges) provided the definitive confirmation. Phase 12's temporal results generalize the principle: replacing prime constants with temporal-period constants yields 7/8 tasks at 100% OOD up to 50-year extrapolation. Phase 13 completes the argument by eliminating all plausible alternative explanations: four established sinusoidal encoding methods all fail to match SPIRNOR's OOD performance, proving the advantage is not from sinusoidal features in general but from *frequency matching to known domain periodicities*. The insight connects neural numeric representation to classical number theory (the Chinese Remainder Theorem), Fourier analysis (exact frequency matching), and a general principle: the best inductive biases are not approximate statistical regularities but exact mathematical invariants of the target domain.
...
*This paper presents results from the SPIRNOR AI experimental research program (Phases 1-13).*
